...
void writePassengerToFile();

void readPassagerFromFile();
...
int main()
{
	...
	readDataFromFile();		//假如有文件系统，就从文件中加载数据（这个函数暂时还没写）
	readPassagerFromFile();
...
void refund()   //退票业务 
{
	...
	if(num==1)
	{
		...
		if(choose>=airlines.num||choose<0)
		{
		    printf("输入有误，请再试一次！");
			getchar();
			getchar();
		...
		while(p->next)
		{
            ...
            {
				t=p->next->passager.booked[p->next->passager.classes-1];
            	n=p->next->passager.classes-1;
            	p->next=p->next->next;	
				flag=1;
			}
			else
		...
		if(flag)
		{
			airlines.airline[temp[choose-1]].leftticketsnum[n] += t;
			airlines.airline[temp[choose-1]].Booked[n] -= t;
			printf("退票成功！");
			writePassengerToFile();
			writeDataToFile();
...
void ChangeInformation(Airline &airline)	//修改某趟航班信息
{
	...
	switch(choose)
	{
		...
		case(5):
		{
			...
			scanf("%d:%d",&airline.date.Hour,&airline.date.Minute);     					   
...
void readDataFromFile() 
{
	...
	fscanf(file,"%d",&airlines.num);
	char c;
	for(int i=0;i<airlines.num;i++)
	{
		Airline *airline = &(airlines.airline[i]);
		fscanf(file,"%s",airline->begging);
		fscanf(file,"%s",airline->arrival);
		fscanf(file,"%s",airline->Airlinenum);
		fscanf(file,"%d",&airline->Airplanenum);
		fscanf(file,"%d.%d.%d-%d:%d\t",&airline->date.Year,
            &airline->date.Mounth,
            &airline->date.Day,
            &airline->date.Hour,
            &airline->date.Minute);
		for (int j = 0; j < 3; j++) {
            fscanf(file, "%d", &airline->passagernum[j]);
        }
		for (int j = 0; j < 3; j++) {
            fscanf(file, "%d", &airline->leftticketsnum[j]);
        }
		for (int j = 0; j < 3; j++) {
            fscanf(file, "%d", &airline->Booked[j]);
        }
		Node *p;
		p=(Node *)malloc(sizeof(Node));
		p->next=NULL;
		airlines.airline[i].passagerlist=p;  	 
    }
...
void readPassagerFromFile()
{
	...
	FILE* file = fopen("passager.txt","w+");	
	if (!file) {
        printf("Failed to open the file: \n");
        return;
    }
    ...
    for(int i;i<airlines.num;i++)
	{
		...
		if(num==i)
			for(int j=0;j<3;j++)
			   for(int k=0;k<airlines.airline[i].Booked[j];k++) 
			   {
 		            Node *p;
 		            char temp[15];
 		            p= (Node *)malloc(sizeof(Node));
			   	    fscanf(file,"%s",temp);
			   	    if(strcmp("EOF",temp)==0)
				        break;
					strcpy(p->passager.name,temp);	 
					fscanf(file,"%d",&p->passager.classes);
					fscanf(file,"%d",&p->passager.booked);
					p->next = airlines.airline[i].passagerlist->next;
					airlines.airline[i].passagerlist->next = p;   
			   }
...
void writeDataToFile() {
    ...
    // 依次写入每个航线的数据
    for (int i = 0; i < airlines.num; i++) {
        const Airline* airline = &(airlines.airline[i]);

        // 写入起点站
        fprintf(file, "%s ", airline->begging);

        // 写入终点站
        fprintf(file, "%s ", airline->arrival);

        // 写入航班号
        fprintf(file, "%s ", airline->Airlinenum);

        // 写入飞机号
        fprintf(file, "%d ", airline->Airplanenum);
        ...
        // 写入成员定额
        for (int j = 0; j < 3; j++) {
            fprintf(file, "%d ", airline->passagernum[j]);
        }

        // 写入余票量
        for (int j = 0; j < 3; j++) {
            fprintf(file, "%d ", airline->leftticketsnum[j]);
        }

        // 写入已订票量
        for (int j = 0; j < 3; j++) {
            fprintf(file, "%d ", airline->Booked[j]);
        }
        fprintf(file, "\n");
    }
...
void writePassengerToFile() {
	...
	for(int i=0;i<airlines.num;i++)
    {
        ...
        fprintf(file,"%d ",i);
        for(int j=0;j<3;j++)
            if(airlines.airline[i].Booked[j]!=0)
			{
				flag=1;
				Node *p;
				p=airlines.airline[i].passagerlist->next;
				while(p)
				{
					fprintf(file,"%s %d %d ",p->passager.name,p->passager.classes,p->passager.booked[j]);
					p=p->next;
				}
			}
